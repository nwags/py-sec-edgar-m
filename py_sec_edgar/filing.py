...
class SecEdgar:
    ...
    def _download(self, filing_url=None, filing_filepath=None, overwrite_if_exists=False):
        try:
            ...
            if not filing_url:
                ...
                logger.debug("Using default filing URL %s", filing_url)

            if not filing_filepath:
                filing_filepath = self.filing_filepath
                logger.debug("Using default filing filepath %s", filing_filepath)
    ...
    def _parse_header(self, raw_html, save_output=False):
                ...
                if len(headerItem) > 0:
                    try:
                        if "<" in headerItem and ">" in headerItem:
                            ...
                            logger.debug("Skipped header item: %s", headerItem)
                    except:
                        keyname = headerItem.strip()
                        valuename = headerItem.strip()
                        logger.debug("Header parse fallback for item: %s", headerItem)

        df_header = pd.DataFrame.from_dict(dict(data), orient='index')
        df_header = df_header.replace('', np.nan)
        ...
        logger.debug("Parsed filing header rows: %s", len(df_header.index))
...
def parse_filing_header(raw_html):
    ...
    header_dict = header_dict.replace('', np.nan)
    ...
    logger.debug("Parsed filing header rows: %s", len(header_dict.index))
    return header_dict
...
def identify_filing(sec_filing_documents, override=None):
    ...
    for i, document in sec_filing_documents.items():
        try:
            logger.debug(
                "DOC %s %s Elements=%s size=%s",
                i,
                document.get('DESCRIPTION'),
                document.get('NUMBER_OF_ELEMENTS'),
    ...
    try:
        if override:
            logger.debug("Override in effect; returning DOC #%s", seq_no - 1)
            seq_no = override
    except:
        pass
